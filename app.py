...
with tab1:
    ...
    if not preds_df.empty and "Points" in preds_df.columns:
        ...
        st.info("ยังไม่มีข้อมูลการทายผล หรือไม่พบคอลัมน์ Points ในไฟล์ predictions.csv")

# ----------------- หน้า 2: ทายผลการแข่งขัน -----------------
with tab2:
    st.header("🔮 กรอกทายผลของคุณ")
    if not preds_df.empty and "User_Email" in preds_df.columns:
        users = preds_df["User_Email"].dropna().unique().tolist()
        user_select = st.selectbox("เลือกชื่อของคุณ (User Email)", users)
    else:
        st.warning("ไม่พบข้อมูลผู้เล่นในระบบ")
        user_select = None
    
    upcoming_matches = matches_df[matches_df["Status"] == "ยังไม่แข่ง"]
    
    if not upcoming_matches.empty and user_select:
...
with tab4:
    ...
    with col_b:
        actual_b = st.number_input(f"สกอร์จริง {curr_match['Team_B']}", min_value=0, max_value=20, step=1, key=f"act_b_{admin_match_id}")
